...
export const connectToModbusSensor = async (modbusClient: ModbusRTU, config: Config): Promise<boolean> => {
    try {
...
export const writeRegister = async (modbusClient: ModbusRTU, addr: number, value: number): Promise<void> => {
    try {
...
export const readRegisters = async (modbusClient: ModbusRTU, addr: number, count: number): Promise<number[]> => {
    try {
