...
export class TemperatureValidator {
    ...
    static validateControlParams(params: any): SensorControlParams {
        ...
        if (params.period !== undefined && typeof params.period === 'number' && params.period >= 100 && params.period <= 10000) {
            validParams.period = params.period;
        }

        if (params.amplitude !== undefined && typeof params.amplitude === 'number' && params.amplitude >= 0 && params.amplitude <= 50) {
    ...
    static validateTimescale(timescale: string): boolean {
        const validTimescales = ['1h', '1hour', '6h', '6hours', '1d', '1day', '7d', '7days', '1w', '1week', '30d', '30days', '1m', '1month'];
