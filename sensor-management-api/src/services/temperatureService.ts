...
export class TemperatureService {
  ...
  async getTemperatureData(
    params: TemperatureQueryParams,
  ): Promise<TemperatureResponse> {
    try {
      // Validate and process query parameters
      const validatedParams =
        TemperatureValidator.validateTemperatureQuery(params);
      ...
      const rawData = await this.temperatureRepository.getTemperatureData(
        timeRange,
        limit,
      );
  ...
  async getTemperatureStats(
    ...
    try {
      ...
      const stats =
        await this.temperatureRepository.getTemperatureStats(timeRange);
  ...
  async getRecentTemperatures(
    limit: number = 100,
  ): Promise<TemperatureResponse> {
    try {
      const data =
        await this.temperatureRepository.getRecentTemperatures(limit);
  ...
  private calculateTimeRange(
    params: TemperatureQueryParams | StatsQueryParams,
  ): TimeRange {
