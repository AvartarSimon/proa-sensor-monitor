...
export class SensorService {
  ...
  async getSensorStatus(): Promise<any> {
    try {
      ...
      console.log('!!!!!!!!!!!!!!!111111111   status', status);
      return {
        ...
        sensorConnection: {
          ...
          lastFailureMessage: status.sensorConnection?.lastFailureMessage || null,
        },
      };
    } catch (error) {
      return {
  ...
  async controlSensor(controlData: any): Promise<ControlResponse> {
    try {
      const validParams = TemperatureValidator.validateControlParams(controlData);
